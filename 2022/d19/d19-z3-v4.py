...
if __name__ == "__main__":
    ...
    for i in range(iterations):
        ...
        s.add(clays - (obsidian_purchases[i] * obsidian_robot_clay_cost) >= 0)
        s.add(obsidians - (geode_purchases[i] * geode_robot_obsidian_cost) >= 0)
        ...
        ores[i + 1] = 1 + sum(ore_purchases[:i])
        clays += sum(clay_purchases[:i])
        obsidians += sum(obsidian_purchases[:i])
